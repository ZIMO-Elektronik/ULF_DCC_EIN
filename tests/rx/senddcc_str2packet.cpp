...
TEST(senddcc_str2packet, string_too_short) {
  EXPECT_FALSE(dcc_ein::rx::senddcc_str2packet("senddcc 00"sv));
  EXPECT_FALSE(dcc_ein::rx::senddcc_str2packet("senddcc 00 01\r"sv));
}

TEST(senddcc_str2packet, string_does_not_start_with_senddcc) {
  EXPECT_FALSE(dcc_ein::rx::senddcc_str2packet("00 01 02 03 04 05 06\r"sv));
}

TEST(senddcc_str2packet, string_does_not_contain_carriage_return) {
  EXPECT_FALSE(dcc_ein::rx::senddcc_str2packet("00 01 02 03 04 05 06 07"sv));
}

TEST(senddcc_str2packet, string_contains_unexpected_chars) {
  EXPECT_FALSE(
    dcc_ein::rx::senddcc_str2packet("senddcc 00 01 0w2 03 04 05 06\r"sv));
}

TEST(senddcc_str2packet, string_valid) {
  EXPECT_EQ(
    dcc_ein::rx::senddcc_str2packet("senddcc 3F 8E 96 5D 38 45 F8 47\r"sv),
...
TEST(senddcc_str2packet, string_data_not_multiple_of_three_chars) {
  EXPECT_FALSE(dcc_ein::rx::senddcc_str2packet("senddcc 3F 8E  96 5D\r"sv));
}

TEST(senddcc_str2packet, string_can_be_longer) {
  EXPECT_EQ(
    dcc_ein::rx::senddcc_str2packet("senddcc A0 57 AC 02 9B\r senddcc 0"sv),

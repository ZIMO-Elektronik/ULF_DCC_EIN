...
TEST(raw2senddcc_str, std_string_and_buffer_versions_are_equal) {
  std::array<uint8_t, 4uz> chunk{0x0Au, 0x12u, 0x49u, 0xFCu};

  // std::string version
  auto std_str{dcc_ein::tx::raw2senddcc_str(chunk)};
  EXPECT_EQ(std_str, "senddcc 0a 12 49 fc\r");

  // preallocated buffer version
  std::array<char, 64uz> buf_str;
  auto last{dcc_ein::tx::raw2senddcc_str(chunk, data(buf_str))};

...
TEST(sendbidi_str2datagram, string_too_short) {
  EXPECT_FALSE(dcc_ein::tx::sendbidi_str2datagram(
    "sendbidi s002a f4 58 81 3b c4 79 df\r"sv));
}

TEST(sendbidi_str2datagram, string_does_not_start_with_sendbidi) {
  EXPECT_FALSE(dcc_ein::tx::sendbidi_str2datagram(
    "sendXXXX s002a f4 58 81 3b c4 79 f5 df\r"sv));
}

TEST(sendbidi_str2datagram, address_identifier_wrong) {
  EXPECT_FALSE(dcc_ein::tx::sendbidi_str2datagram(
    "sendbidi x002a f4 58 81 3b c4 79 f5 df\r"sv));
}

TEST(sendbidi_str2datagram, BLA_BLA) {
  auto retval{dcc_ein::tx::sendbidi_str2datagram(
